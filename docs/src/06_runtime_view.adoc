...
note over Client, Redis: Triggered immediately after player move response (game not over)
Client -> WebApp : move registered on board
WebApp -> GM : POST /reqBotMove
GM -> Redis : Get Current State
GM -> Engine : Request Bot Move (Bot Server)
Engine --> GM : Bot Move Result + New State
GM -> Redis : Update State (Cache)
GM --> WebApp : Bot coordinates + game_over
WebApp --> Client : Board updated with bot move
@enduml
....
